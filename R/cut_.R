...
#' 
#' @param fmt see functions [cut_levels()] and \link[base]{sprintf}
#' 
...
cut_ <- function(
  ...
  breaks <- c(breaks, -Inf, Inf) |> # `breaks` before `Inf` and `-Inf`, then the returned value has attributes of `breaks`
    ...
    sort() # *not* ?base::sort.int # I need to retain the attr of `breaks`
  ...
  attr(ret, which = 'levels') <- cut_levels(breaks = breaks, ..., fmt = fmt, data.name = data.name)
  class(ret) <- 'factor'
  return(ret)
...
#' @title [cut_levels()]
#' 
#' @param breaks,right,include.lowest see functions \link[base]{cut.default} and \link[base]{.bincode}.
#' 
#' @param fmt see function \link[base]{sprintf}
#' 
#' @param data.name \link[base]{character} scalar or \link[base]{language}, see function [cut_()]
#' 
#' @examples 
#' foo = \(...) {
#'  cbind(
...
#'  )
#' }
#' foo(breaks = c(-Inf, 1:4, Inf), right = TRUE, include.lowest = TRUE)
#' foo(breaks = c(-Inf, 1:4, Inf), right = FALSE, include.lowest = TRUE)
#' foo(breaks = c(-Inf, 1:4, Inf), right = TRUE, include.lowest = FALSE)
#' foo(breaks = c(-Inf, 1:4, Inf), right = FALSE, include.lowest = FALSE)
#' @keywords internal
#' @export
cut_levels <- function(
    breaks, 
